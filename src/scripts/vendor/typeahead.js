...
(function (root, factory) {
	...
	var PersistentStorage = function () {
		...
		_.mixin(PersistentStorage.prototype, {
			_prefix: function (key) {return this.prefix + key;},
			...
			set: function (key, val, ttl) {
				if (_.isNumber(ttl)) {this._safeSet(this._ttlKey(key), encode(now() + ttl));}
		...
		function gatherMatchingKeys(keyMatcher) {
			...
			for (i = 0; i < len; i++) {
				if ((key = LOCAL_STORAGE.key(i)).match(keyMatcher)) {keys.push(key.replace(keyMatcher, ""));}
			}
			return keys;
		}
	}();
		...
		_.mixin(Bloodhound.prototype, {
			...
			search: function search(query, sync, async) {
				...
				function processRemote(remote) {
					var nonDuplicates = [];
					_.each(remote, function (r) {!_.some(local, function (l) {return that.identify(r) === that.identify(l);}) && nonDuplicates.push(r);});
...
(function (root, factory) {
	if (typeof define === "function" && define.amd) {define("typeahead.js", ["jquery"], function (a0) {return factory(a0);});}
	else if (typeof exports === "object") {module.exports = factory(require("jquery"));}
	else {factory(jQuery);}
})(this, function ($) {
	var _ = function () {
	...
	var EventBus = function () {
		...
		_.mixin(EventBus.prototype, {
			...
			trigger: function (type) {
				...
				if (deprecatedType =
						deprecationMap[type]) {this._trigger(deprecatedType, [].slice.call(arguments, 1));}
			}
		});
		return EventBus;
	}();
	var EventEmitter = function () {
		...
		function getNextTick() {
			...
			if (window.setImmediate) {
				nextTickFn = function nextTickSetImmediate(fn) {setImmediate(function () {fn();});};
			}
			else {nextTickFn = function nextTickSetTimeout(fn) {setTimeout(function () {fn();}, 0);};}
			return nextTickFn;
		}

		function bindContext(fn, context) {
			return fn.bind ? fn.bind(context) : function () {fn.apply(context, [].slice.call(arguments, 0));};
		}
	}();
	var highlight = function (doc) {
		...
		function getRegex(patterns, caseSensitive, wordsOnly) {
			...
			for (var i = 0, len = patterns.length; i < len; i++) {escapedPatterns.push(_.escapeRegExChars(patterns[i]));}
			regexStr =
				wordsOnly ? "\\b(" + escapedPatterns.join("|") + ")\\b" : "(" + escapedPatterns.join("|") + ")";
		...
		_.mixin(Dataset.prototype, EventEmitter, {
			...
			_getSuggestionsFragment: function getSuggestionsFragment(query, suggestions) {
				...
				_.each(suggestions, function getSuggestionNode(suggestion) {
					...
					$el =
						$(that.templates.suggestion(context)).data(keys.obj, suggestion).data(keys.val, that.displayFn(suggestion)).addClass(that.classes.suggestion + " " + that.classes.selectable);
			...
			update: function update(query) {
				...
				function async(suggestions) {
					...
					if (!canceled && rendered < that.limit) {
						that.cancel = $.noop;
						var top = Math.max(0, that.limit - rendered);
						top = Math.min(top, suggestions.length);
						rendered += suggestions.length;
						that._append(query, suggestions.slice(0, top));
	...
	var Menu = function () {
		...
		_.mixin(Menu.prototype, EventEmitter, {
			_onSelectableClick: function onSelectableClick($e) {this.trigger("selectableClicked", $($e.currentTarget));},
			...
			_ensureVisible: function ensureVisible($el) {
				...
				nodeHeight =
					this.$node.height() + parseInt(this.$node.css("paddingTop"), 10) + parseInt(this.$node.css("paddingBottom"), 10);
	...
	var DefaultMenu = function () {
		...
		_.mixin(DefaultMenu.prototype, Menu.prototype, {
			...
			_onRendered: function onRendered() {
				if (this._allDatasetsEmpty()) {this._hide();}
			...
			_show: function show() {this.$node.css("display", "inline-block");}
		});
		return DefaultMenu;
	}();
	var Typeahead = function () {
		...
		_.mixin(Typeahead.prototype, {
			...
			_onEnterKeyed: function onEnterKeyed(type, $e) {
				var $selectable;
				if ($selectable =
						this.menu.getActiveSelectable()) {this.select($selectable) && $e.preventDefault();}
			},
			_onTabKeyed: function onTabKeyed(type, $e) {
				...
				if ($selectable =
						...
						this.menu.getTopSelectable()) {this.autocomplete($selectable) && $e.preventDefault();}
			},
			...
			initialize: function initialize(o, datasets) {
				...
				function attach() {
					...
					if (defaultHint || defaultMenu) {
						...
						$input.wrap($wrapper).parent().prepend(defaultHint ? $hint : null).append(defaultMenu
							? $menu : null);
					}
